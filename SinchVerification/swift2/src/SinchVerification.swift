...
public protocol Verification {
    ...
    */
    func initiate(completion: (Bool, NSError?) -> Void);
    ...
    - parameter completion: Closure that will be invoked upon completion.
    */
    func verify(code: String, completion: (Bool, NSError?) -> Void);
    ...
    */
    func completionQueue(queue: dispatch_queue_t);
    ...
    */
    func environmentHost(host: String);
...
*/
public func SMSVerification(applicationKey applicationKey: String, phoneNumber: String) -> Verification {
    let objcVerification = SINVerification.SMSVerificationWithApplicationKey(applicationKey, phoneNumber: phoneNumber);
...
/**
 ...
 - returns: A new SMS Verification object.
 */
public func SMSVerification(applicationKey applicationKey: String, phoneNumber: String, custom: String) -> Verification {
    let objcVerification = SINVerification.SMSVerificationWithApplicationKey(applicationKey, phoneNumber: phoneNumber, custom:custom);
    return VerificationWrapper(objcVerification);
}
...
*/
public func CalloutVerification(applicationKey applicationKey: String, phoneNumber: String) -> Verification {
    let objcVerification = SINVerification.calloutVerificationWithApplicationKey(applicationKey, phoneNumber: phoneNumber);
    return VerificationWrapper(objcVerification);
}
...
/**
 ...
 - returns: A new Callout Verification object.
 */
public func CalloutVerification(applicationKey applicationKey: String, phoneNumber: String, custom:String) -> Verification {
    let objcVerification = SINVerification.calloutVerificationWithApplicationKey(applicationKey, phoneNumber: phoneNumber, custom:custom);
    return VerificationWrapper(objcVerification);
}
...
*/
public class SinchVerification {
...
*/
public enum PhoneNumberParseError: ErrorType {
...
public protocol PhoneNumberUtil {
    ...
    */
    func parse(input: String, defaultRegion: String) throws -> PhoneNumber;
    ...
    */
    func format(phoneNumber: PhoneNumber, format: PhoneNumberFormat) -> String;
    ...
    */
    func exampleNumber(isoCountryCode: String) -> PhoneNumber?;
    ...
    */
    func isPossibleNumber(string: String, fromRegion: String) -> (possible: Bool, error: PhoneNumberValidationError?);
    ...
    /**
     ...
     the input is not considered a viable phone number.
     */
    func isPossibleNumber(phoneNumber: PhoneNumber) -> (possible: Bool, error: PhoneNumberValidationError?);
    ...
    */
    func regionList(forLocale locale:NSLocale) -> SINRegionList;
}
...
public func SharedPhoneNumberUtil() -> PhoneNumberUtil {
    ...
    struct Static {
        ...
        static var instance: PhoneNumberUtil? = nil;
    }
    dispatch_once(&Static.once, {
        Static.instance = PhoneNumberUtilWrapper(SINPhoneNumberUtil());
    });
...
public final class TextFieldPhoneNumberFormatter {
    ...
    */
    public func exampleNumber(format format:PhoneNumberFormat) -> String? {
        return self.objcFormatter.exampleNumberWithFormat(ConvertPhoneNumberFormat(format));
    }

    private var objcFormatter: SINUITextFieldPhoneNumberFormatter;
}
...
internal class VerificationWrapper: Verification {
    ...
    init(_ verification: SINVerificationProtocol) {
        ...
        if objcVerification.respondsToSelector(setIsSwiftFlavor){
            self.objcVerification.performSelector(setIsSwiftFlavor);
        }
    }
    
    func initiate(completion: (Bool, NSError?) -> Void) {
        self.objcVerification.initiateWithCompletionHandler({ (success: Bool, error: NSError?) -> Void in
            completion(success, error);
        });
    }
    
    func verify(code: String, completion: (Bool, NSError?) -> Void) {
        self.objcVerification.verifyCode(code, completionHandler: { (success: Bool, error: NSError?) -> Void in
            completion(success, error);
        });
    }
    ...
    private var objcVerification: SINVerificationProtocol;
...
internal class PhoneNumberWrapper : PhoneNumber {
    ...
    var countryCode : Int { return self.objcPhoneNumber.countryCode.integerValue; }
    
    var rawInput : String? { return self.objcPhoneNumber.rawInput }
    
    private var objcPhoneNumber: SINPhoneNumber;
}
...
internal class PhoneNumberUtilWrapper: PhoneNumberUtil {
    ...
    private var objcUtil : SINPhoneNumberUtilProtocol;
    ...
    func exampleNumber(isoCountryCode: String) -> PhoneNumber? {
        if let phoneNumber = self.objcUtil.exampleNumberForRegion(isoCountryCode) {
    ...
    func mapValidationError(nsError: NSError) -> PhoneNumberValidationError {
    ...
    func regionList(forLocale locale:NSLocale) -> SINRegionList {
        return self.objcUtil.regionListWithLocale(locale);
    }
}


internal func ConvertPhoneNumberFormat(format: PhoneNumberFormat) -> SINPhoneNumberFormat {
    switch(format){
    case .E164:
